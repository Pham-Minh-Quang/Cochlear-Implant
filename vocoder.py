...
# tauEnvMS to remove carrier synthesis effect
    taus = tauEnvMS/1000
    alpha = np.exp(-1/(taus*captFs)) #taus = 10/1000 ; captTs = 55556
...
# Define auxilliary variables    
    ...
    np.savetxt("audioPwr.csv", audioPwr, delimiter=",",fmt='%30f')
    ...
    #print(normOffset)
    #np.savetxt("normOffset.csv", normOffset, delimiter=",")
    #print(nl)
    #print(alpha)
    #print(elData)
    #np.savetxt("elData.csv", elData, delimiter=",")
    # print(np.floor(elData.shape[1]/blkSize))
    # print(normRamp)
    # np.savetxt("normRamp.csv", normRamp, delimiter=",")
...
#%% Loop through frames of electrode data, convert to electric field, calculate neural spectrum
    ...
    timeIdx = np.arange((40-1)*blkSize+1,40*blkSize+1,dtype=int)-1
        #timeIdx = np.arange(278) # dòng này thêm vào test
        #print(timeIdx)
        #timeIdx.tofile('timeIdx.csv',sep=',',format='%10.5f')
        #np.savetxt("timeIdx_[10842-11119].csv", timeIdx, delimiter=",",fmt='%30f')
        #abc=elData[:,timeIdx]
        #print(elData[:,timeIdx])
        #np.savetxt("elData[timeIdx].csv", abc, delimiter=",")
        
    efData = np.dot(normRamp,elData[:,timeIdx])
    # print(efData)
    #np.savetxt("efData[10842-11119].csv", efData, delimiter=",",fmt='%30f')
        
        # Normalized EF to neural activity        
    activity = ElFieldToActivity(efData,normOffset,nl,nlExp)  # JIT optimized
    # print(activity)
    np.savetxt("activity[10842-11119]P.csv", activity, delimiter=",",fmt='%30f')
    ...
    #print(audioPwr)      
    np.savetxt("audioPwr[10842-11119]P.csv", audioPwr, delimiter=",",fmt='%30f')
        # # Average energy
